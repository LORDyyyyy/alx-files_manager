    ...
    return new Promise((resolve) => {
      MongoClient.connect(
        ...
        async (err, client) => {
          ...
          await testClientDb
            .collection('users')
            .insertOne({ email: 'me@me.com' });
          await testClientDb
            .collection('users')
            .insertOne({ email: 'me2@me.com' });
      ...
      .end((err, res) => {
        ...
        testClientDb
          ...
          .toArray((err, docs) => {
            ...
            chai
              .expect(docUser.password.toUpperCase())
              .to.equal(initialUser.password.toUpperCase());
            done();
          });
      });
  }).timeout(30000);
      ...
      .end((err, res) => {
        ...
        testClientDb
          ...
          .toArray((err, docs) => {
            ...
            chai
              .expect(docUser.password.toUpperCase())
              .to.equal(sha1(userParam.password).toUpperCase());
            done();
          });
      });
  }).timeout(30000);
...
describe('------------------\nTASK 4\nGET /connect', () => {
  ...
  it('GET /connect with invalid Base64 content', (done) => {
    const basicAuth = `Basic ${Buffer.from(`hello`, 'binary').toString(
      'base64',
    )}`;
    chai
      .request('http://localhost:5000')
...
describe('GET /connect', () => {
  ...
  it('GET /connect with unknown user email', (done) => {
    const basicAuth = `Basic ${Buffer.from(
      `fake_${initialUser.email}:${initialUserPwd}`,
      'binary',
    ).toString('base64')}`;
    chai
      .request('http://localhost:5000')
...
describe('GET /connect', () => {
  ...
  it('GET /connect  with wrong user password', (done) => {
    const basicAuth = `Basic ${Buffer.from(
      `${initialUser.email}:${initialUserPwd}_wrong`,
      'binary',
    ).toString('base64')}`;
    chai
      .request('http://localhost:5000')
...
describe('GET /connect', () => {
  ...
  it('GET /connect with correct user email and password', (done) => {
    const basicAuth = `Basic ${Buffer.from(
      `${initialUser.email}:${initialUserPwd}`,
      'binary',
    ).toString('base64')}`;
    chai
      .request('http://localhost:5000')
      ...
      .end(async (err, res) => {
        ...
        testClientDb
          ...
          .toArray((err, docs) => {
            ...
            const docFile = docs.filter(
              (i) => i._id.toString() == resFile.id.toString(),
            )[0];
            chai.expect(docFile.name).to.equal(fileData.name);
            chai.expect(docFile._id.toString()).to.equal(resFile.id);
            chai.expect(docFile.userId.toString()).to.equal(initialUserId);
...
describe('POST /files', () => {
  ...
  const folderTmpFilesManagerPath =
      ...
      .end(async (err, res) => {
        ...
        testClientDb
          ...
          .toArray((err, docs) => {
            ...
            if (fs.existsSync(folderTmpFilesManagerPath)) {
              newFilesInTmpFilesManager = fs.readdirSync(
                folderTmpFilesManagerPath,
              );
            }
            chai
              .expect(newFilesInTmpFilesManager.length)
              .to.equal(filesInTmpFilesManager.length + 1);
            const newFileInDiskPath = newFilesInTmpFilesManager.filter(
              (x) => !filesInTmpFilesManager.includes(x),
            );

            const newFileInDiskContent = fs
              .readFileSync(
                `${folderTmpFilesManagerPath}/${newFileInDiskPath[0]}`,
              )
              .toString();
    ...
    chai
      ...
      .end(async (err, res) => {
        ...
        resList.forEach((item) => {
          ...
          chai.expect(itemInit.type).to.equal(item.type);
          chai
            .expect(itemInit.parentId.toString())
            .to.equal(item.parentId.toString());
        });
    ...
    return new Promise((resolve) => {
      ...
      MongoClient.connect(
        ...
        async (err, client) => {
          ...
          const createdUserFileDocs = await testClientDb
            .collection('users')
            .insertOne(fileUser);
      ...
      MongoClient.connect(
        ...
        async (err, client) => {
          ...
          const createdUFolderDocs = await testClientDb
            .collection('files')
            .insertOne(initialUnpublishedFolder);
          if (createdUFolderDocs && createdUFolderDocs.ops.length > 0) {
            initialUnpublishedFolderId =
              createdUFolderDocs.ops[0]._id.toString();
          }
          ...
          const createdPFolderDocs = await testClientDb
            .collection('files')
            .insertOne(initialPublishedFolder);
    ...
    return new Promise((resolve) => {
      ...
      MongoClient.connect(
        ...
        async (err, client) => {
          ...
          const createdFilePDocs = await testClientDb
            .collection('files')
            .insertOne(initialFileP);
          ...
          const createdFileUPDocs = await testClientDb
            .collection('files')
            .insertOne(initialFileUP);
